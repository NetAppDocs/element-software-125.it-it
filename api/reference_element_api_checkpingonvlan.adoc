...
[role="lead"]
È possibile utilizzare il `CheckPingOnVlan` metodo per verificare la connettività di rete su una VLAN temporanea quando si esegue la convalida di rete pre-distribuzione. `CheckPingOnVlan` Crea un'interfaccia VLAN temporanea, invia pacchetti ICMP a tutti i nodi del cluster di archiviazione utilizzando l'interfaccia VLAN, quindi rimuove l'interfaccia.
