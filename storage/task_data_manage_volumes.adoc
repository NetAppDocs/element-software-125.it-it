...
NOTE: I sistemi operativi differiscono nel modo in cui gestiscono i volumi clonati. VMware ESXi tratterà un volume clonato come una copia del volume o un volume snapshot. Il volume sarà un dispositivo disponibile da usare per creare un nuovo datastore. Per maggiori informazioni sul montaggio dei volumi clonati e sulla gestione delle LUN snapshot, consulta la documentazione VMware su  https://techdocs.broadcom.com/["Montaggio di una copia del datastore VMFS"^] e  https://techdocs.broadcom.com/["Gestione di datastore VMFS duplicati"^].
